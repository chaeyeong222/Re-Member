...
public class OAuth2LoginSuccessHandler implements AuthenticationSuccessHandler {
    ...
    public void onAuthenticationSuccess(HttpServletRequest request, HttpServletResponse response,
        ...
        try {
            ...
            if (oAuth2User.getRole() == Role.CUSTOMER) {

                String accessToken = jwtService.createAccessToken(oAuth2User.getPhone());
                response.addHeader(jwtService.getAccessHeader(), "Bearer " + accessToken);
                response.sendRedirect("http://localhost:3000/sign-up"); // 프론트의 회원가입 추가 정보 입력 폼으로 리다이렉트

                jwtService.sendAccessAndRefreshToken(response, accessToken, null);
                // Role을 Guest에서 User로
                User findUser = userRepository.findBySocialId(oAuth2User.getPhone())
    ...
    private void loginSuccess(HttpServletResponse response, CustomOAuth2User oAuth2User) throws IOException {
        ...
        jwtService.updateRefreshToken(oAuth2User.getPhone(), refreshToken);
    }
}